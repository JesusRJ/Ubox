...
//
// See "UboxPing.h" for purpose, syntax, version history, links, and more.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// UboxPing constructor
// ---------------------------------------------------------------------------

UboxPing::UboxPing(uint8_t trigger_pin, uint8_t echo_pin, unsigned int max_cm_distance) {
...
unsigned long UboxPing::ping_cm() {
	unsigned long echoTime = UboxPing::ping();         // Calls the ping method and returns with the ping echo distance in uS.
#if ROUNDING_ENABLED == false
	return (echoTime / US_ROUNDTRIP_CM);              // Call the ping method and returns the distance in centimeters (no rounding).
#else
	return UboxPingConvert(echoTime, US_ROUNDTRIP_CM); // Convert uS to centimeters.
#endif
}


unsigned long UboxPing::ping_in() {
	unsigned long echoTime = UboxPing::ping();         // Calls the ping method and returns with the ping echo distance in uS.
#if ROUNDING_ENABLED == false
	return (echoTime / US_ROUNDTRIP_IN);              // Call the ping method and returns the distance in inches (no rounding).
#else
	return UboxPingConvert(echoTime, US_ROUNDTRIP_IN); // Convert uS to inches.
#endif
}


unsigned long UboxPing::ping_median(uint8_t it) {
...
void UboxPing::ping_timer(void (*userFunc)(void)) {
	if (!ping_trigger()) return;         // Trigger a ping, if it returns false, return without starting the echo timer.
	timer_us(ECHO_TIMER_FREQ, userFunc); // Set ping echo timer check every ECHO_TIMER_FREQ uS.
}


boolean UboxPing::check_timer() {
...
void UboxPing::timer_us(unsigned int frequency, void (*userFunc)(void)) {
...
void UboxPing::timer_ms(unsigned long frequency, void (*userFunc)(void)) {
	intFunc = UboxPing::timer_ms_cntdwn;  // Timer events are sent here once every ms till user's frequency is reached.
	...
	TIMSK4 = (1<<TOIE4);   // Enable Timer4 interrupt.
#elif defined (__arm__) && defined (TEENSYDUINO) // Timer for Teensy 3.x
	itimer.begin(UboxPing::timer_ms_cntdwn, 1000);  // Set timer to 1ms (1000 uS).
#else
...
void UboxPing::timer_stop() { // Disable timer interrupt.
#if defined (__AVR_ATmega32U4__) // Use Timer4 for ATmega32U4 (Teensy/Leonardo).
	TIMSK4 = 0;
#elif defined (__arm__) && defined (TEENSYDUINO) // Timer for Teensy 3.x
	itimer.end();
...
unsigned int UboxPing::convert_cm(unsigned int echoTime) {
#if ROUNDING_ENABLED == false
...
unsigned int UboxPing::convert_in(unsigned int echoTime) {
#if ROUNDING_ENABLED == false
