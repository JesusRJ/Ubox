// ---------------------------------------------------------------------------
// UboxPing Library - v1.7 - 09/29/2015
//
// AUTHOR/LICENSE:
...
// available ping and ultrasonic libraries causing the problem. The UboxPing
...
//
// CONSTRUCTOR:
//   UboxPing sonar(trigger_pin, echo_pin [, max_cm_distance])
...
//   UboxPing::timer_us(frequency, function) - Call function every frequency microseconds.
//   UboxPing::timer_ms(frequency, function) - Call function every frequency milliseconds.
//   UboxPing::timer_stop() - Stop the timer.
//
// HISTORY:
...
//   you're also using UboxPing's ping_timer because both use Timer2 interrupts).
...
//   doesn't give correct results (at least with ping sensors). The UboxPing
...
#define UboxPingConvert(echoTime, conversionFactor) (max(((unsigned int)echoTime + conversionFactor / 2) / conversionFactor, (echoTime ? 1 : 0)))
...
class UboxPing {
	public:
		UboxPing(uint8_t trigger_pin, uint8_t echo_pin, unsigned int max_cm_distance = MAX_SENSOR_DISTANCE);
		unsigned int ping();
